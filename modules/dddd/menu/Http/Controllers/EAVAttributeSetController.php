...
namespace DTV\EAVAttribute\Http\Controllers;

use DTV\EAVAttribute\Models\EavAttribute;
use DTV\EAVAttribute\Models\EavAttributeSet;
use DTV\EAVAttribute\Selectable\AttributeGroup;
...
class EAVAttributeSetController extends AdminController
{
    ...
    {
        Permission::check('dtv.eav-attribute-set');
        ...
        $grid->actions(function (Grid\Displayers\Actions $actions) {
            if (!Admin::user()->can('dtv.eav-attribute-set.delete') || $actions->row->is_system) {
    ...
    {
        Permission::check('dtv.eav-attribute-set.create');
        return $content
            ->title($this->title())
    ...
    {
        Permission::check('dtv.eav-attribute-set.edit');
        return $content
            ->title($this->title())
    ...
    public function destroy($id)
    {
        try {
            if (!$this->eavAttributeSet->find($id)->is_system) {
                Permission::check('dtv.eav-attribute-set.delete');
