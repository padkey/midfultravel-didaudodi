...
class EAVAttributeGroupController extends AdminController
{
    ...
    {
        Permission::check('dtv.eav-attribute-group');
        ...
        $grid->actions(function (Grid\Displayers\Actions $actions) {
            if (!Admin::user()->can('dtv.eav-attribute-group.delete') || $actions->row->is_system) {
    ...
    {
        Permission::check('dtv.eav-attribute-group.create');
        return $content
            ->title($this->title())
    ...
    {
        Permission::check('dtv.eav-attribute-group.edit');
        return $content
            ->title($this->title())
    ...
    public function destroy($id)
    {
        try {
            if (!$this->eavAttributeGroup->find($id)->is_system) {
                Permission::check('dtv.eav-attribute-group.delete');
