...
use DDDD\EAVAttribute\Database\Seeders\DatabaseSeeder;
use DDDD\EAVAttribute\Observers\EavAttributeGroupObserver;
use DDDD\EAVAttribute\Observers\EavAttributeSetObserver;
...
class EAVAttributeServiceProvider extends ServiceProvider
{
    ...
    public function boot(EAVAttribute $extension)
    {
        ...
        if ($this->app->runningInConsole() && $assets = $extension->assets()) {
            $this->publishes(
                [$assets => public_path('vendor/DDDD/eav-attribute')],
                'eav-attribute'
            );
        }
